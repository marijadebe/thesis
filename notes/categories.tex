...
\end{tikzcd}$ for $f\in\homc{\catc}{A}{B}$
            \item for each $A,B,C\in\ob{\catc}$ an assignment $\circ$ : $\homc{\catc}{B}{C}\times\homc{\catc}{A}{B}\to \homc{\catc}{A}{C}$ such that
...
\begin{definition}
    ...
    \begin{enumerate}
        \item an assignment $\ob{\catc}\to\ob{\catd}$ taking $A \mapsto F(A)$
        \item for each $A,B\in\ob{\catc}$ an assignment $\homc{\catc}{A}{B}\to\homc{\catd}{F(A)}{F(B)}$ which takes $f \mapsto F(f)$
    \end{enumerate}
