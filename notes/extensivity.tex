...
\[
    F_+((A_1,f_1: A_1 \to X_1),(A_2,f_2: A_2 \to X_2)) = 
    (A_1+ A_2, (\iota_{X_1}\circ f_1) + (\iota_{X_2}\circ f_2))
\]
...
and that the two big rectangles commute
\[\begin{tikzcd}
	{A_1} && {A_1+A_2} && {A_2} \\
	{B_1} && {B_1+B_2} && {B_2} \\
	{C_1} && {C_1+C_2} && {C_2}
...
\begin{proposition}
	...
	it has pullbacks along all insertions of binary coproducts and
	given that the following diagram commutes
\[\begin{tikzcd}
	{A_1} && A && {A_2} \\
	\\
	{X_1} && {X_1+X_2} && {X_2}
	\arrow["{\omega_1}", from=1-1, to=1-3]
	\arrow["{{f_1}}"{description}, from=1-1, to=3-1]
	\arrow["f"{description}, from=1-3, to=3-3]
	\arrow["{\omega_2}"', from=1-5, to=1-3]
	\arrow["{{f_2}}"{description}, from=1-5, to=3-5]
	\arrow["{{\iota_{X_1}}}", from=3-1, to=3-3]
	\arrow["{{\iota_{X_2}}}"', from=3-5, to=3-3]
\end{tikzcd}\]
	then $A_1$ and $A_2$ are a pair of pullbacks if and only if 
	$A$ is a coproduct of $A_1$ and $A_2$ in the category of $\catc /(X_1+X_2)$
	$,\omega_1,\omega_2$ are insertions and $f=f_1+f_2$.
\end{proposition}
\begin{proof}[Proof]
