\chapter{Diagrams of acts}
    In this chapter we state and prove different diagram lemmata for pointed acts. The proposition statements and proofs are 
...
\begin{proof}[Proof]
    ...
    In the first case, because of Rees exactness at $B'$,
    we find $x_0,y_0\in A'$ such that $f'(x_0)=x$ and $f'(y_0)=y$, which implies 
    $f''(\alpha'(x_0))=\beta'(x)=\beta'(y)=f''(\alpha'(y_0))$. If $\alpha'(x_0)=\alpha'(y_0)$, then by Rees regularity 
    of $\alpha'$ we would have $\alpha'(x_0)=0=\alpha'(y_0) \implies \beta'(x)=0=\beta'(y)$, since $x_0\neq y_0$.
    If $\alpha'(x_0)\neq\alpha'(y_0)$, then by Rees regularity of $f''$ we still have $\beta'(x)=f''(\alpha'(x_0))=0=f''(\alpha'(y_0))=\beta'(y)$.\par
...
\begin{proposition}[Splitting sequences]
    ...
    \begin{enumerate}
        ...
        $g\circ\phi^{-1}=\lambda_C$, where $\iota_A : A \to A\coprod C$, $x\mapsto (x,0)$ and 
        $\lambda_C : A\coprod C \to C$, $(x,y)\mapsto y$.
    \end{enumerate}
\end{proposition}
...
\[\begin{tikzcd}
	...
	\arrow[from=1-4, to=1-5]
\end{tikzcd}\]
    (where by $0$ we mean $\cdots\to 0\to0\to0\to\cdots$) is \textbf{Rees short exact} 
